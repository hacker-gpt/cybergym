...
export class ProductDetailsComponent implements OnInit, OnDestroy {
  ...
  public reviewControl: UntypedFormControl = new UntypedFormControl("", [
  ...
  addReview(textPut: HTMLTextAreaElement) {
    ...
    this.productReviewService
      .create(this.data.productData.id, review)
      .subscribe({
        next: () => {
          this.reviews$ = this.productReviewService.get(
            this.data.productData.id,
          );
        },
        error: (err) => {
          console.log(err);
  ...
  editReview(review: Review) {
    this.dialog
      .open(ProductReviewEditComponent, {
        width: "500px",
        height: "max-content",
        data: {
          reviewData: review,
